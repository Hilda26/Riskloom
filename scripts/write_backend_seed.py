"""
StableShield AI - Seed Data Writer
Writes backend/supabase/seed/seed_stablecoins.sql (STEP 5).
Run from the repository root: python scripts/write_backend_seed.py
...
SEED_DIR = ROOT / "backend" / "supabase" / "seed"
SEED_FILE = SEED_DIR / "seed_stablecoins.sql"
...
def main() -> None:
    ...
    if SEED_FILE.exists():
        print(f"Already exists, skipping: {SEED_FILE}")
        return
